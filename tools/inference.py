        ...
        if self.conf["env"]["draw_results"]:
            ...
            with multiprocessing.Pool(multiprocessing.cpu_count() // 2) as pools:
                ...
                pools.map(
                    ...
                    zip(
                        itertools.repeat(utils.draw_landmark), x_img, output_np, itertools.repeat(self.save_dir), img_id
                    ),
                )
        ...
        if self.conf["env"]["draw_results"]:
            with multiprocessing.Pool(multiprocessing.cpu_count() // 2) as pools:
                ...
                pools.map(
                    utils.multiprocessing_wrapper,
                    zip(
                        itertools.repeat(utils.draw_image),
                        x_img,
                        output_prob,
                        itertools.repeat(self.save_dir),
                        img_id,
                        itertools.repeat(self.conf["model"]["num_class"]),
                    ),
                )
        ...
        for idx in range(x_img.shape[0]):
            ...
            for i in range(self.conf["model"]["num_class"]):
                ...
                utils.append_data_stats(
                    self.data_stat,
                    key_name,
                    len(torch.where(output_argmax == i)[0])
                    / (output["seg"][idx].shape[-2] * output["seg"][idx].shape[-1]),
                )
