...
class TrainerBase:
    ...
    def __init__(self, conf, now=None, k_fold=0):
        self.conf = conf
        self.accelerator = Accelerator()  # `$accelerate config` to set configuration at first
        ...
        if not self.conf["env"]["debug"]:
            ...
            shutil.copy(
                ...
                os.path.join(self.saved_model_directory, os.path.split(self.conf["config_path"])[-1]),
            )

            # init logger file
            utils.Logger(
                os.path.join(self.saved_model_directory, "log.txt"),
        ...
        # init model with pretrained weight
        ...
        if self.conf["model"]["saved_ckpt"] != "":
            try:
                ...
                utils.Logger().info(f"{utils.Colors.LIGHT_RED}Model loaded successfully.{utils.Colors.END}")
            except RuntimeError:
                self.model.module.load_pretrained_imagenet(self.conf["model"]["saved_ckpt"])
                utils.Logger().info(
                    f"{utils.Colors.LIGHT_RED}Model loaded alternatively using `load_pretrained_imagenet` function.{utils.Colors.END}"
                )

        # init model EMA
        self.model_ema = None
        if self.conf["model"]["use_ema"]:
            self.model_ema = timm.utils.ModelEmaV2(self.model, decay=0.9995, device=self.device)

        # init dataloader
        self.loader_train = utils_tool.init_data_loader(conf=self.conf, conf_dataloader=self.conf["dataloader_train"])
        self.loader_valid = utils_tool.init_data_loader(conf=self.conf, conf_dataloader=self.conf["dataloader_valid"])

        # init optimizer
        self.optimizer = utils_tool.init_optimizer(self.conf["optimizer"], self.model)

        # init scheduler
        self.scheduler = utils_tool.set_scheduler(
            self.conf, self.conf["dataloader_train"], self.optimizer, self.loader_train
        )
        ...
        # set acceleration
        self.model, self.optimizer, self.scheduler, self.loader_train, self.loader_valid = self.accelerator.prepare(
            self.model, self.optimizer, self.scheduler, self.loader_train, self.loader_valid
        )
    ...
    def _backward_and_update_weight(self, optimizer, loss, scheduler=None):
        ...
        if self.model_ema is not None:
            # self.model_ema.update(self.model if not hasattr(self.model, "module") else self.model.module)
    ...
    def save_model(self, model, epoch, metric=None, metric_name="metric"):
        if not self.conf["env"]["debug"]:
            ...
            # remove post model and save current model
    ...
    def evaluate_model(self, model, epoch, metric_dict):
        for key in metric_dict.keys():
            # if key does not exist in the past (for epoch 1)
            if key not in self.metric_best.keys():
                self.metric_best[key] = metric_dict[key]
                self.save_model(model, epoch, metric_dict[key], metric_name=key)

            # if new metric outperforms the best metric
            if ("loss" in key and metric_dict[key] < self.metric_best[key]) or (
                "loss" not in key and metric_dict[key] > self.metric_best[key]
            ):
                self.metric_best[key] = metric_dict[key]
                self.save_model(model, epoch, metric_dict[key], metric_name=key)

    def end_train(self):
        utils.Logger().info("The model seems to be converged.")
        for key in self.metric_best.keys():
            utils.Logger().info(f"{utils.Colors.LIGHT_RED}Best {key} -----> {self.metric_best[key]}{utils.Colors.END}")
            if self.conf["env"]["wandb"]:
                wandb.log({f"Best {key}": self.metric_best[key]})
