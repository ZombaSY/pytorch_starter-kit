...
from models import lr_scheduler, metrics, model_implements, utils
...
def set_scheduler(conf, conf_dataloader, optimizer, data_loader):
    ...
    if hasattr(conf, "scheduler"):
        if conf["scheduler"]["name"] == "WarmupCosine":
            scheduler = lr_scheduler.WarmupCosineSchedule(
                optimizer=optimizer,
            ...
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer, T_max=conf["scheduler"]["cycles"], eta_min=conf["scheduler"]["lr_min"]
            )
            ...
            scheduler = lr_scheduler.WarmupConstantSchedule(
                optimizer, warmup_steps=steps_per_epoch * conf["scheduler"]["warmup_epoch"]
            )
        else:
            utils.Logger().info(
                f"{utils.Colors.LIGHT_PURPLE}No scheduler found --> {conf['scheduler']['name']}{utils.Colors.END}"
            )
    else:
        pass
