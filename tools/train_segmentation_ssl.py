...
class TrainerSegmentationSSL(TrainerSegmentation):
    def __init__(self, conf, now=None, k_fold=0):
        ...
        self.loader_train_ssl = utils_tool.init_data_loader(conf=self.conf, conf_dataloader=self.conf["dataloader_ssl"])
        ...
        self.scheduler_ssl = utils_tool.set_scheduler(
            self.conf, self.conf["dataloader_ssl"], self.optimizer_ssl, self.loader_train_ssl
        )
        ...
        for epoch in range(1, self.conf["env"]["epoch"] + 1):
            ...
            if (epoch - self.last_saved_epoch) > self.conf["env"]["early_stop_epoch"]:
                ...
                if self.conf["env"]["wandb"]:
                    wandb.log({"Best mIoU": self.metric_best["mIoU"]})
                break
