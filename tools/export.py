...
class Exportor:
    ...
    def torch2onnx(self):
        ...
        torch.onnx.export(
            self.model, dummy_input, self.save_dir + ".onnx", opset_version=self.conf["export"]["opset_version"]
        )
    ...
    def torchjit2coreml(self):
        ...
        mlmodel = ct.convert(
            self.traced_model,
            inputs=[
                ct.ImageType("input", shape=self.conf["export"]["input_shape"], scale=scale, bias=bias)
            ],  # expects ct.TensorType
