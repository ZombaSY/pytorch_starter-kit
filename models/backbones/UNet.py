...
class Down(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1, bias=False):
        ...
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2), DoubleConv(in_channels, out_channels, kernel_size=kernel_size, padding=padding, bias=bias)
        )
...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True, kernel_size=3, padding=1, bias=False):
        ...
        if bilinear:
            ...
            )  # set align_corners=False in tflite version
            self.conv = DoubleConv(
                in_channels, out_channels, in_channels // 2, kernel_size=kernel_size, padding=padding, bias=bias
            )
        else:
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out_dict = {"seg": logits, "feats": [x2, x3, x4, x5]}
