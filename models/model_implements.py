...
from models.backbones import MobileNet_v4, MobileOne, MobileOneNet, Swin
from models.backbones import UNet_light as UNet_light_parts
from models.backbones import timm_backbone
from models.heads import MLP, UPerHead
...
class Swin_t(nn.Module):
    def __init__(self, conf_model, base_c=96):
        ...
        self.swin_transformer = Swin.SwinTransformer(
...
class Swin_t_semanticSegmentation_dsv(Swin_t):
    def __init__(self, conf_model, base_c=96):
        ...
        self.uper_head = UPerHead.M_UPerHead_dsv(
...
class UNet_light_dsv(nn.Module):
    def __init__(self, conf_model):
        ...
        self.u_net = UNet_light_parts.UNet_light_dsv(
            n_channels=conf_model["in_channel"],
    ...
    def forward(self, x):
        ...
        for i in range(len(out_dict["seg_aux"])):
            out_dict["seg_aux"][i] = F.interpolate(
                out_dict["seg_aux"][i], (x_h, x_w), mode="bilinear", align_corners=False
            )
...
class UNet_light_regression_v4(nn.Module):
    def __init__(self, conf_model):
        super().__init__()
        self.u_net = UNet_light_parts.UNet_light(
            n_channels=conf_model["in_channel"],
            n_classes=conf_model["num_class"],
            base_c=conf_model["base_c"],
            bilinear=True,
        ...
        self.u_net_backbone = UNet_light_parts.UNet_Backbone(
            n_channels=conf_model["in_channel"],
            n_classes=conf_model["num_class"],
            base_c=conf_model["base_c"],
            bilinear=True,
        ...
        self.classifier = MLP.Classifier_map_score_multi_stem_only_feat(
            dims=conf_model["base_c"] * 8, dim_inter=conf_model["base_c"] * 16
        )
        self.train_callback()
...
class UNet_light_regression(nn.Module):
    def __init__(self, conf_model):
        super().__init__()
        self.backbone = UNet_light_parts.UNet_Backbone(
            n_channels=conf_model["in_channel"],
            n_classes=conf_model["num_class"],
            base_c=conf_model["base_c"],
            bilinear=True,
...
class ConvNextV2_l_landmark(nn.Module):
    def __init__(self, conf_model):
        ...
        self.classifier = MLP.SimpleLandmarker(
            channel_in=1536, num_points=conf_model["num_class"], bottleneck_size=[4, 4]
        )
...
class Mobileone_s0_landmark(nn.Module):
    def __init__(self, conf_model):
        ...
        self.classifier = MLP.SimpleLandmarker(
            channel_in=1024, num_points=conf_model["num_class"], bottleneck_size=[4, 4]
        )
...
class Mobilenetv4_t_landmark(nn.Module):
    def __init__(self, conf_model):
        ...
        self.classifier = MLP.SimpleLandmarker(
            channel_in=960, num_points=conf_model["num_class"], bottleneck_size=[4, 4]
        )
...
class MobileOneNet_v1_landmark_light(nn.Module):
    def __init__(self, conf_model):
        super().__init__()
        self.backbone = MobileOneNet.mobileonenetv1()
        self.classifier = MLP.SimpleLandmarker_light(
            channel_in=960, num_points=conf_model["num_class"], bottleneck_size=[4, 4]
        )
